var parentWidth = document.getElementById("linechart").offsetWidth;

var margin = {top: 15, right: (parentWidth * 0.1), bottom: 250, left: (parentWidth * 0.1)},
...
var color = d3.scale.ordinal().range(["#F2DE8A", "#56AE7C", "#cc0066", "#0066cc", "#FF07F1"]);
...
d3.tsv("./data/Weather3.tsv", function (error, data) {
    ...
    var categories = color.domain().map(function (name) { // Nest the data into an array of objects with new keys
        ...
        return {
            ...
            visible: (name === "Average Temperature (C)" ? true : false) // "visible": all false except for economy which is true.
        };
    });
    ...
    issue.append("rect")
            ...
            .attr("y", function (d, i) {
                if( i === 1 || i === 3){
                    return (300 + legendSpace) + 1 * (legendSpace);
                }else{
                    return (300 + legendSpace) + 2 * (legendSpace);
                }
    ...
    issue.append("text")
            .attr("x", function(d,i){
                if(i < 2){
                    ...
                    return legendbox * 2 + width/2;
                }
    })
            .attr("y", function (d, i) {
                if( i === 1 || i === 3){
                    return (310 + legendSpace) + 1 * (legendSpace);
                }else{
                    return (310 + legendSpace) + 2 * (legendSpace);
                }
    ...
    function mousemove() {
        ...
        var format = d3.time.format("%e/%m/%Y"); // Format hover date text to show three letter month and full year
    ...
    function brushed() {
        ...
        var min = d3.time.format.iso(brush.extent()[0]);
        var max = d3.time.format.iso(brush.extent()[1]);
        redrawMap(min, max);
    }
    ;
